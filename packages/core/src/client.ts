...
export class MedplumClient extends TypedEventTarget<MedplumClientEventMap> {
   ...
   */
  private commitLogin(login: LoginState): void {
  ...
  private async refreshProfile(state?: RequestState): Promise<WithId<ProfileResource> | undefined> {
    ...
    this.profilePromise = new Promise((resolve, reject) => {
      this.request<SessionDetails>(
        'auth/me',
        { method: 'GET', cache: 'no-cache', signal: createAuthRequestSignal() },
        state
      )
  ...
  private async runRefresh(gracePeriod?: number, force = false): Promise<void> {
    ...
    const run = async (): Promise<LoginState | undefined> => {
      ...
      // Commit before releasing the lock, so the next tab in line reads the rotated refresh token.
