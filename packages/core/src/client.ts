...
export class MedplumClient extends EventTarget {
   ...
   */
  private async request<T>(url: string, options: RequestInit = {}, state: RequestState = {}): Promise<T> {
    ...
    if ((response.status === 200 || response.status === 201) && redirectMode === 'follow') {
      ...
      if (contentLocation) {
        // Follow redirect
        return this.request(contentLocation, { ...options, body: undefined });
      }
    }
   ...
   */
  private handleUnauthenticated(url: string, options: RequestInit): Promise<any> {
    if (this.refresh()) {
      return this.request(url, options);
    }
    this.clearActiveLogin();
